	...
	// 如果在目录下输入为空，则跳转到指定链接
		...
		@RequestMapping(value="/notice/list",method=RequestMethod.GET)
		 public String index(Model model,String content){
			...
			if (content!=null){
				job_list = rainservice.get_NoticeLikeList("title",content);
			}
			model.addAttribute("list",job_list);
			return "notice/list";
		}
		@RequestMapping(value="/notice/now",method=RequestMethod.GET)
		 public String now(Model model,String content){
			Date date = new Date();    
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			content = sdf.format(date);
			List<Notice> job_list = rainservice.get_NoticeList();
			if (content!=null){
				job_list = rainservice.get_NoticeLikeList("date",content);
			}
			model.addAttribute("list",job_list);
			return "notice/now";
		}
